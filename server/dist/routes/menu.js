...
// -----------------------------
async function fetchTenantIdByCode(app, tenantCode) {
...
async function fetchCategories(app, tenantId) {
...
async function fetchPublicItems(app, tenantId, categories) {
    const { data, error } = await app.supabase
        .from('menu_items')
...
export default async function menuRoutes(app) {
    ...
    app.get('/menu/public', async (req, reply) => {
        const qs = z.object({ tenantCode: z.string().min(1) }).safeParse(req.query);
        if (!qs.success) {
    ...
    app.post('/menu/items:bulk', {
        ...
        const parsed = BulkCsvSchema.safeParse(req.body);
        if (!parsed.success) {
            return reply.code(400).send({ error: 'invalid_body', details: parsed.error.flatten() });
        }
        const tenantId = req.auth?.primaryTenantId;
        ...
        try {
            ...
            const { data: allCats } = await app.supabase
                ...
                .select('id, name')
                .eq('tenant_id', tenantId);
            const byName = new Map((allCats || []).map((c) => [String(c.name).trim().toLowerCase(), c.id]));
            for (let i = 1; i < lines.length; i++) {
                const values = lines[i].split(',').map((v) => v.trim());
                try {
                    const item = { tenant_id: tenantId };
