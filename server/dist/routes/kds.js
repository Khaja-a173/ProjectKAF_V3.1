...
export default async function kdsRoutes(app) {
    ...
    app.get('/kds/orders', async (req, reply) => {
        ...
        try {
          ...
          `)
            ...
            if (parsed.data.lane) {
                return reply.send({ [parsed.data.lane]: lanes[parsed.data.lane] });
            }
    ...
    // --------------------------------------------------------------------------
    app.post('/kds/orders/:id/advance', async (req, reply) => {
        if (!ENABLE_KDS_RT) {
            return reply.code(503).send({ error: 'KDS features disabled', reason: 'feature_flag_off' });
        }
        ...
        try {
            ...
            const updates = { status: body.data.to_status };
            const nowISO = new Date().toISOString();
            if (body.data.to_status === 'ready')
                updates.ready_at = nowISO;
            if (body.data.to_status === 'served')
                updates.served_at = nowISO;
            ...
            app.log.info(`KDS: Order ${params.data.id} advanced from ${order.status} to ${body.data.to_status}`);
    ...
    // --------------------------------------------------------------------------
    app.get('/kds/latest', async (req, reply) => {
        if (!ENABLE_KDS_RT) {
            return reply.code(503).send({ error: 'KDS features disabled', reason: 'feature_flag_off' });
        }
        const tenantId = req.auth?.primaryTenantId;
