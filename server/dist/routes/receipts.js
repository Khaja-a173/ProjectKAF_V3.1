...
export default async function receiptsRoutes(app) {
    // ------------------------------------------------------------
    ...
    app.post('/receipts/send', { preHandler: [app.requireAuth] }, async (req, reply) => {
        const body = SendReceiptSchema.safeParse(req.body);
        if (!body.success) {
        ...
        try {
            ...
            if (!channel)
                return reply.code(400).send({ error: 'channel_required' });
    ...
    // ------------------------------------------------------------
    app.post('/receipts/invoice/:orderId', { preHandler: [app.requireAuth] }, async (req, reply) => {
        const tenantId = req.auth?.primaryTenantId;
        if (!tenantId)
    ...
    app.post('/receipts/print', { preHandler: [app.requireAuth] }, async (req, reply) => {
        ...
        try {
            ...
            try {
                ...
                if (pjErr)
                    app.log.warn({ pjErr }, 'print_jobs insert failed, continuing with mock');
            }
            catch {
                // ignore
            }
    ...
    app.post('/printer/config', { preHandler: [app.requireAuth] }, async (req, reply) => {
        const body = PrinterConfigSchema.safeParse(req.body);
        if (!body.success)
            return reply.code(400).send({ error: 'invalid_body', details: body.error.flatten() });
        const tenantId = req.auth?.primaryTenantId;
        if (!tenantId)
