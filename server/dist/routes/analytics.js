...
function getDateLabel(date, granularity) {
    switch (granularity) {
        case 'day':
...
export default async function analyticsRoutes(app) {
    ...
    app.get('/analytics/summary', { preHandler: [app.requireAuth] }, async (req, reply) => {
        ...
        const db = req.supabaseUser || app.supabase;
    ...
    app.get('/analytics/revenue-series', { preHandler: [app.requireAuth] }, async (req, reply) => {
        ...
        if (!validW.includes(window))
            return reply.code(400).send({ error: 'invalid_window' });
        if (!validG.includes(granularity))
    ...
    app.get('/analytics/revenue-breakdown', { preHandler: [app.requireAuth] }, async (req, reply) => {
        ...
        const by = qs.by?.toLowerCase();
        const granularity = qs.granularity || qs.interval || 'day';
        if (!bucket || !by)
        ...
        let startTime = new Date(bucketDate);
        let endTime = new Date(bucketDate);
        switch (granularity) {
            case 'day':
    ...
    app.get('/analytics/revenue_timeseries', { preHandler: [app.requireAuth] }, async (req, reply) => {
        ...
        const validW = ['7d', '30d', '90d', 'mtd', 'qtd', 'ytd'];
        const validG = ['day', 'week', 'month'];
        if (!validW.includes(range))
            return reply.code(400).send({ error: 'invalid_range' });
        if (!validG.includes(interval))
    ...
    app.get('/analytics/revenue_breakdown', { preHandler: [app.requireAuth] }, async (req, reply) => {
        ...
        const bucket = qs.bucket;
        const by = qs.by?.toLowerCase();
        const interval = qs.interval || 'day';
        if (!bucket || !by)
            return reply.code(400).send({ error: 'missing_params' });
        if (!['item', 'category', 'order_type'].includes(by)) {
            return reply.code(400).send({ error: 'invalid_by' });
